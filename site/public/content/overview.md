...
## 6. 회의 결정 반영 (2026-09-09)
| 항목 | 결정 |
|---|---|
| 회원 범위 | 학부 + 기존 대학원(MBA 제외). 박사 포함 여부는 추가 결정 |
| 상대 총동창회 관계 | 병립 — 별도 조직, 상호 링크·공동 행사 |
| 브랜드 | 서울대학교 경영대학 총동문회 + SNU Business School Alumni |
| 디자인 | 버건디 · 골드 · 아이보리 유지 |
| 인증 | 가입 신청 → 사무국 명부 대조 승인 → 이메일 로그인 |
| 공개 범위 | 기본 이름·학번·소속, 연락처 opt-in |
| 커뮤니티 | 로그인 전용 그룹 (재확인 필요) |
| 알림 | 카카오톡 채널 + 이메일 |
| 영문 | 전체 영문 사이트 /en/ |
| 작업대 보호 | Cloudflare Access, 운영진 이메일에 오너가 권한 부여 |
| 오픈 | 핵심 20명 즉시 → 11/26 송년회에서 준비모임 200명 공개(사실상 정식). QR 준비 |
| 보류 | 회비 체계(숫자 리서치 후), 결제(다중 결제수단 리서치), 기부자 예우(기금위원회), 멘토링 |

## 7. 현재 상태 · 다음 할 일
- **완료(P0·P1)**: 리서치, IA, 디자인 시스템, 공개 사이트, 작업대 8페이지, snubusiness.com 개통, 회의 결정 저장
- **진행(P2, ~11/26)**: 브랜드 영문명 교체, Access 보호, /en/, 회비·결제 리서치, 자료 수령 시 반영
- **대기**: 운영위원회 자료(임원·회칙·회비·사무국·사진·행사), 단톡방 논의(관계 문구·동문 기업·개인정보 책임자)
